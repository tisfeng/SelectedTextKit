...
- 新增或修改了 `SelectedTextKitExampleTests/**/*.swift` 下的测试源码。
- 用户明确要求构建或测试。
...
- 库源码变更使用 `swift build --target SelectedTextKit` 验证 SwiftPM target。
- `swift build` 构建全部 Package targets；依赖需要解析时使用 `swift package resolve`。
- `Package.swift` 没有 test target；`swift test` 不能替代示例工程中的测试。
- Accessibility、AppleScript、浏览器和剪贴板测试可能依赖系统权限、前台应用或真实
  剪贴板；记录这些环境依赖，选择确定性 fixture，不将环境缺口写为验证通过。
...
xcodebuild test-without-building \
  -project SelectedTextKitExample.xcodeproj \
  -scheme SelectedTextKitExample \
  -only-testing:SelectedTextKitExampleTests/<TestSuiteOrClass>

# Run one test method
xcodebuild test-without-building \
  -project SelectedTextKitExample.xcodeproj \
  -scheme SelectedTextKitExample \
  -only-testing:SelectedTextKitExampleTests/<TestSuiteOrClass>/<testMethod>
```
...
- Swift 源码发生变化时使用仓库已经配置的格式化工具；当前没有仓库级 formatter
  配置，不凭空要求安装或运行工具，匹配周围代码风格。
