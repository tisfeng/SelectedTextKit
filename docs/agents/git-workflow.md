...
## SelectedTextKit PR 交付

需要创建 PR 时使用通用的 `submit-pr` skill。根据实际目标 checkout 动态发现 base
repository、默认分支、base remote、head remote 与 fork 关系，不硬编码 `main`、`dev`
或 `origin`。用户明确指定时传入对应参数；歧义按 skill 流程处理。

Issue 策略使用 skill 的通用默认值 `neutral`，用户另有要求时显式传入
`--issue-policy`。缺少 PR 模板时使用内置四段式正文，不要求项目创建模板文件。
PR review 遵循 `.agents/skills/review-pr/SKILL.md` 的完整流程。
